...
export function CartPage() {
  ...
  useEffect(() => {
    if (slug) fetchStore(slug).then(setStore).catch(() => {});
  }, [slug]);

  const whatsappNumber = store?.info.whatsappNumber;

  function handleOrderViaWhatsApp() {
    if (!whatsappNumber || !store) return;
          ...
          <>
              ...
              {items.map((item) => (
                <div key={item.productId} className="flex items-center gap-3 rounded-2xl bg-white p-3 shadow-card">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm font-bold text-brand-600">{item.price.toLocaleString()} IQD</p>
                  </div>
            ...
            <div className="mt-6 rounded-2xl bg-white p-4 shadow-card">
              <div className="flex items-center justify-between text-sm text-slate-500">
                <span>Subtotal</span>
                <span className="font-semibold text-slate-800">{subtotal.toLocaleString()} IQD</span>
              </div>
              <div className="mt-1 flex items-center justify-between text-base font-bold text-slate-800">
                <span>Total</span>
                <span className="text-brand-600">{subtotal.toLocaleString()} IQD</span>
              </div>
            </div>
            ...
            {whatsappNumber ? (
              <button
                ...
                className="mt-6 flex w-full items-center justify-center gap-2 rounded-2xl bg-emerald-500 py-3.5 text-sm font-bold text-white shadow-card transition hover:bg-emerald-600"
              >
                Order via WhatsApp
              </button>
            ) : (
