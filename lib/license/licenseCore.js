...
function buildMessage(deviceId, limitToken) {
  return `${coreFromDeviceId(deviceId)}|${limitToken}`;
}

function formatLicenseCode(deviceId, limitToken, signatureBytes) {
  const deviceIdCore = coreFromDeviceId(deviceId);
  const sigB64 = bytesToBase64(signatureBytes);
  return `${PREFIX}${limitToken}-${deviceIdCore}-${sigB64}`;
}

// Shape-only parsing — no crypto here. Returns { ok:false } if the code doesn't even
// look like a license code.
function parseLicenseCode(rawCode) {
  const code = (rawCode || '').trim().replace(/\s+/g, '');
  if (!code.startsWith(PREFIX)) return { ok: false };

  const rest = code.slice(PREFIX.length); // "<limitToken>-<deviceIdCore>-<sigB64>"
  const firstDash = rest.indexOf('-');
  if (firstDash === -1) return { ok: false };
  const limitToken = rest.slice(0, firstDash);
  if (!TOKEN_TO_LIMIT[limitToken]) return { ok: false };

  const afterToken = rest.slice(firstDash + 1); // "<deviceIdCore>-<sigB64>"
  const secondDash = afterToken.indexOf('-');
  if (secondDash === -1) return { ok: false };
  const deviceIdCore = afterToken.slice(0, secondDash);
  const sigB64 = afterToken.slice(secondDash + 1);
  ...
  return { ok: true, limitToken, deviceIdCore, signatureBytes };
}

// Pure function: does NOT touch storage. Returns one of:
//   { status: 'invalid' }
//   { status: 'wrong_device' }
//   { status: 'valid', limitToken }
function verifyLicenseCode(rawCode, myDeviceId) {
  ...
  const message = buildMessage(myDeviceId, parsed.limitToken);
  ...
  return { status: 'valid', limitToken: parsed.limitToken };
}
