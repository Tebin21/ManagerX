  ...
  const paymentsRows = payments.length === 0
    ...
    : payments.map((p) => `
        <tr>
          ...
          <td style="color:#1E40AF;font-weight:600;">${fmtIQD(p.remainingAfter)} IQD</td>
        </tr>
      `).join('');

  const settled = debt.remainingAmount <= 0;
  const fillColor = settled ? '#059669' : '#1E40AF';
...
<div class="page">
  ...
  <div class="header" style="background:linear-gradient(135deg,#1E3A8A 0%,#2563EB 60%,#3B82F6 100%);">
  ...
  <div class="footer">
    <p>Generated by <strong>${escHtml(business.name)}</strong> &middot; ManagerX &middot; ${formatDateTime(new Date().toISOString())}</p>
  </div>
