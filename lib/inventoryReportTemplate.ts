...
export function buildInventoryReportHTML(
  ...
  const rows = products.map((p) => {
    ...
    const sellPriceStyle = isKurdish
      ...
      : 'text-align:right;color:#2563EB;font-weight:600;';
...
<div class="page">
  ...
  <div class="body">
    ...
    <div class="summary-row">
      ${summaryCard(isKurdish ? escHtml(t('totalProducts')) : 'Total Products', fmtIQD(stats.totalProducts), '#EFF6FF', '#1E3A8A', '#BFDBFE', isKurdish)}
      ${summaryCard(isKurdish ? escHtml(t('totalQuantity')) : 'Total Quantity', fmtIQD(stats.totalQuantity), '#F0FDF4', '#166534', '#BBF7D0', isKurdish)}
      ${summaryCard(isKurdish ? escHtml(t('totalValueIQD')) : 'Total Value (IQD)', `${fmtIQD(stats.totalValueIQD)} IQD`, '#EFF6FF', '#1E40AF', '#BFDBFE', isKurdish)}
      ...
      <table>
        ...
        <tfoot>
          <tr>
            ...
            <td style="direction:ltr;text-align:left;color:${isKurdish ? '#0F172A' : '#1E3A8A'};font-size:14px;">${fmtIQD(stats.totalValueIQD)} IQD</td>
            <td colspan="2"></td>
          </tr>
        </tfoot>
      </table>
  ...
  <div class="footer">
    ${isKurdish ? escHtml(t('generatedBy')) : 'Generated by'} ${isKurdish ? escHtml(business.name) : `<strong>${escHtml(business.name)}</strong>`} &middot; ManagerX &middot; ${dateStr} at ${timeStr}
  </div>
