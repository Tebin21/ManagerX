...
<div class="page">
  ...
  <div class="footer">
    ${isKurdish ? escHtml(t('generatedBy')) : 'Generated by'} ${isKurdish ? escHtml(business.name) : `<strong>${escHtml(business.name)}</strong>`} &middot; ${isKurdish ? `<span dir="rtl">${escHtml(pdfDevelopedByText())}</span>` : pdfDevelopedByText()} · ${PDF_BRAND_WEBSITE} &middot; ${dateStr} at ${timeStr}
  </div>
