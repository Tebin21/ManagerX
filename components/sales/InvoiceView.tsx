...
const PAYMENT_TEXT: Record<string, string> = {
  cash: '#065F46', fib: '#1E40AF', debt: '#991B1B',
};
    ...
    <View>
      ...
      <View style={styles.headerRow}>
        ...
        <View style={[styles.payBadge, { backgroundColor: PAYMENT_BG[sale.paymentMethod] }]}>
          <Text style={[styles.payBadgeText, { color: PAYMENT_TEXT[sale.paymentMethod] }]}>
      ...
      {(sale.customerName || sale.customerPhone) && (
        <View style={styles.section}>
          ...
          {sale.customerName ? <Text style={styles.infoText}>{sale.customerName}</Text> : null}
          {sale.customerPhone ? <Text style={styles.infoSub}>{sale.customerPhone}</Text> : null}
          {sale.customerAddress ? <Text style={styles.infoSub}>{sale.customerAddress}</Text> : null}
        </View>
      )}
      ...
      {sale.warranty ? (
        <View style={styles.section}>
          ...
          <Text style={styles.infoText}>{sale.warranty}</Text>
        </View>
      ) : null}
          ...
          {items.map((item, i) => (
            <View key={item.id ?? i} style={styles.itemRow}>
              <View style={styles.itemLeft}>
                ...
                {item.itemId ? <Text style={styles.itemId}>#{item.itemId}</Text> : null}
              </View>
      ...
      <View style={[styles.section, styles.totalsSection]}>
        ...
        {sale.discountTotal > 0 && (
          <View style={styles.totalRow}>
            ...
            <Text style={[styles.totalValue, { color: Colors.success }]}>−{fmt(sale.discountTotal)}</Text>
          </View>
        )}
        ...
        {sale.paymentMethod === 'debt' && (
          <>
            <View style={styles.totalRow}>
              ...
              <Text style={[styles.totalValue, { color: Colors.success }]}>{fmt(sale.paidAmount)}</Text>
            </View>
            {sale.remainingDebt > 0 && (
              <View style={styles.totalRow}>
                <Text style={[styles.totalLabel, { color: Colors.error }]}>Remaining Debt</Text>
                <Text style={[styles.totalValue, { color: Colors.error, fontWeight: '700' }]}>
                  {fmt(sale.remainingDebt)}
                </Text>
              </View>
            )}
          </>
      ...
      {sale.notes ? (
        <View style={styles.section}>
          ...
          <Text style={styles.infoSub}>{sale.notes}</Text>
        </View>
      ) : null}
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  itemId: { fontSize: 11, color: Colors.primary, marginTop: 1 },
